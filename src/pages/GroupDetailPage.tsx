...
import type { Group, StudentWithPayment, PaymentsSummary, AttendanceRecord, AttendanceStatus } from '../types'
...
export default function GroupDetailPage() {
  ...
  // Load existing attendance from API, or default all students to PRESENT
  const loadAttendance = useCallback(async () => {
    ...
    try {
      ...
      // On error, still default all to PRESENT
      if (data) {
        const map: Record<number, AttendanceStatus> = {}
  ...
  // Toggle attendance: ON (PRESENT/LATE) <-> OFF (ABSENT)
  function toggleAttendance(studentId: number) {
    setAttendanceDirty(true)
    setLocalAttendance(prev => {
      const current = prev[studentId] || 'PRESENT'
      // If currently PRESENT or LATE -> switch to ABSENT; if ABSENT -> switch to PRESENT
      const next: AttendanceStatus = current === 'ABSENT' ? 'PRESENT' : 'ABSENT'
      return { ...prev, [studentId]: next }
    })
  }

  // Toggle late status (only when student is present)
  function toggleLate(studentId: number) {
    setAttendanceDirty(true)
    setLocalAttendance(prev => {
      const current = prev[studentId] || 'PRESENT'
      if (current === 'ABSENT') return prev // can't be late if absent
      return { ...prev, [studentId]: current === 'LATE' ? 'PRESENT' : 'LATE' }
    })
  }
  ...
  async function saveAttendance() {
    ...
    try {
      const records = Object.entries(localAttendance).map(([sid, status]) => ({
  ...
  async function handleEditStudentSubmit(ev: React.FormEvent) {
    ...
    try {
      await api.patch(`/api/students/${editStudent.id}`, {
        ...
        customMonthlyFee: editStudentCustomMonthlyFee ? Number(editStudentCustomMonthlyFee) : null,
      })
  ...
  async function createMonthPayments() {
    ...
    try {
      await api.post('/api/payments/create-month', {
        groupId: Number(id),
        date: paymentStartDate || (paymentMonth + '-01'),
      })
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex border-b border-slate-200 mt-6 mb-4">
        <button
          className={`pb-2 px-4 font-medium text-sm transition-colors ${activeTab === 'payments' ? 'border-b-2 border-indigo-600 text-indigo-700' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
          onClick={() => setActiveTab('payments')}
        >
          To'lovlar
        </button>
        <button
          className={`pb-2 px-4 font-medium text-sm transition-colors ${activeTab === 'attendance' ? 'border-b-2 border-indigo-600 text-indigo-700' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              onClick={() => {
                setPaymentMonth(month.slice(0, 7))
                setPaymentStartDate(month.slice(0, 7) + '-01')
                ...
                {students.map(s => (
                  <StudentRow
                    ...
                    onEdit={s => {
                      ...
                      setEditStudentPhone(s.parentPhone || '')
                      setEditStudentPaymentStartDate(s.paymentStartDate ? s.paymentStartDate.slice(0, 10) : '')
                      setEditStudentCustomMonthlyFee(s.customMonthlyFee ? String(s.customMonthlyFee) : '')
                    }}
                  />
                ))}
              </div>
          ...
          <div className="card p-4">
            <div className="flex items-center gap-3">
              <input
                ...
                className="input-base flex-1 sm:flex-none sm:w-auto"
                disabled={attendanceSaving}
              />
              {attendanceDirty && (
                <span className="text-xs text-amber-600 font-medium">● Saqlanmagan</span>
              )}
            </div>
          </div>
          ...
          <div className="card">
            {attendanceLoading ? (
              <div className="flex items-center justify-center py-10">
                ...
                <span className="ml-2 text-sm text-[var(--text-secondary)]">Yuklanmoqda...</span>
              </div>
            ) : activeStudents.length === 0 ? (
              <p className="text-sm text-[var(--text-secondary)] text-center py-8">Guruhda faol o'quvchi yo'q</p>
            ) : (
                ...
                {activeStudents.map(s => {
                  ...
                  return (
                    <div key={s.id} className="flex items-center gap-3 px-4 py-3">
                      {/* Student name + status label */}
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-sm text-[var(--text-primary)] truncate">{s.name}</p>
                        <p className={[
                          'text-xs font-medium mt-0.5',
                          status === 'PRESENT' ? 'text-emerald-600' :
                            status === 'LATE' ? 'text-amber-600' : 'text-rose-500',
                        ].join(' ')}>
                          {status === 'PRESENT' ? 'Bor' : status === 'LATE' ? 'Kechikdi' : "Yo'q"}
                        </p>
                      </div>

                      {/* Late chip — only visible when toggle is ON */}
                      {isPresent && (
                        <button
                          type="button"
                          ...
                          disabled={attendanceSaving}
                          className={[
                            'px-2.5 py-1 rounded-full text-[11px] font-semibold transition-all shrink-0',
                            isLate
                              ? 'bg-amber-100 text-amber-700 ring-1 ring-amber-300 dark:bg-amber-900/30 dark:text-amber-400'
                              : 'bg-[var(--bg-page)] text-[var(--text-muted)] hover:bg-amber-50 hover:text-amber-600',
                          ].join(' ')}
                        >
                          Kech
                        </button>
                      ...
                      <button
                        ...
                        disabled={attendanceSaving}
                        className={[
                          'relative inline-flex h-7 w-12 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2',
                          isPresent ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600',
                          attendanceSaving ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer',
                        ].join(' ')}
                      >
                        <span
                          className={[
            ...
            {/* Save button */}
            {activeStudents.length > 0 && !attendanceLoading && (
              <div className="px-4 py-3 border-t border-[var(--border-color)]">
                <Button
                  ...
                  disabled={!attendanceDirty && !Object.keys(localAttendance).length}
                >
                  Davomatni saqlash
                </Button>
              </div>
      ...
      {/* Monthly Payment Creation Modal */}
      <Modal open={showPaymentModal} onClose={() => setShowPaymentModal(false)} title="Oylik to'lov yaratish">
        <div className="space-y-4">
          <p className="text-sm text-[var(--text-secondary)]">
            Guruh uchun oylik to'lovlarni yaratish. Har bir o'quvchiga individual yoki guruh narxi bo'yicha to'lov yaratiladi.
          </p>
          <div>
            <label className="block text-sm font-medium text-[var(--text-primary)] mb-1">To'lov oyi</label>
            <input
              type="month"
              value={paymentMonth}
              onChange={e => {
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-primary)] mb-1">To'lov boshlanish sanasi</label>
            <input
              type="date"
              ...
              className="input-base w-full"
            />
            <p className="text-xs text-[var(--text-muted)] mt-1">Bu sana to'lov davri boshlanishini belgilaydi</p>
          </div>
          <div className="flex gap-3 pt-2">
            <Button type="button" variant="secondary" fullWidth onClick={() => setShowPaymentModal(false)}>
              Bekor
            </Button>
            <Button fullWidth onClick={createMonthPayments} loading={creatingPayments}>
              Yaratish
            </Button>
          </div>
        </div>
      ...
      <Modal open={showAddStudent} onClose={() => { setShowAddStudent(false); setNameError('') }} title="O'quvchi qo'shish">
        <form onSubmit={handleAddStudent} className="space-y-4" noValidate>
          ...
          />
          <Input
            label="To'lov boshlanish sanasi (ixtiyoriy)"
            type="date"
          ...
          />
          <Input
            label="Maxsus oylik to'lov (ixtiyoriy)"
            type="number"
            value={newCustomMonthlyFee}
            onChange={e => setNewCustomMonthlyFee(e.target.value)}
            placeholder="Asosiy guruh narxi o'rniga"
            disabled={addingStudent}
          />
      ...
      <Modal open={!!editStudent} onClose={() => setEditStudent(null)} title="O'quvchini tahrirlash">
        <form onSubmit={handleEditStudentSubmit} className="space-y-4" noValidate>
          <Input
            label="Ism"
          ...
          <Input
            label="To'lov boshlanish sanasi (ixtiyoriy)"
            type="date"
            value={editStudentPaymentStartDate}
            onChange={e => setEditStudentPaymentStartDate(e.target.value)}
          />
          <Input
            label="Maxsus oylik to'lov (ixtiyoriy)"
            type="number"
            value={editStudentCustomMonthlyFee}
            onChange={e => setEditStudentCustomMonthlyFee(e.target.value)}
            placeholder="Asosiy guruh narxi o'rniga"
          />
          <div className="flex gap-3 pt-1">
            <Button type="button" variant="secondary" fullWidth onClick={() => setEditStudent(null)}>
              Bekor
            </Button>
